...
function EditItem({ data }: Props) {
    const [category, setCategory] = useState<string>(data.category || "")
    const [menuForm, setMenuForm] = useState<MenuForm>(data || {
        name: "",
        category: "",
        price: 0,
        cost: 0,
    ...
    const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            // Assume you have the id of the document you want to update
            const docId = data.id;
            await updateDoc(doc(db, "menu", docId), newForm);
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button size="sm" variant="ghost" className="w-full justify-start">Edit</Button>
            </DialogTrigger>
